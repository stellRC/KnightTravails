...
class Knight {
  ...
  moveKnight(start, end, board) {
    ...
    while (moves.length !== 0) {
      ...
      const nextSquare = board.getPossiblePath(currentSquare[0], currentSquare[1]);
      nextSquare.forEach((square) => {
        if (board.playedMove[square[0]][square[1]]) return;
        moves.push(square);
        board.playedMove[square[0]][square[1]] = true;
        board.lastSquare[square[0]][square[1]] = [currentSquare[0], currentSquare[1]];
      });
    }
  ...
  printPath(path) {
    console.log(`=> You made it in ${path.length} spectacular moves!  Here's your path:`);
